...
enum WorkloadClientChecks {
    static func run() async -> [String] {
        ...
        check(
            snapshot.authoredPullRequests.first?.reviewers.map(\.displayName) == ["alice", "dave", "carol", "erin"],
            "Review roster contains requests and review decisions while excluding the Pull request author",
            failures: &failures
        )
